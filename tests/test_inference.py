...
def test_normal_carving_rho_one_matches_truncgauss():
    # rho->1 with sigma_x=sigma_y means X and Y coincide, so conditioning on
    # Y > c is the same as conditioning on X > c directly -- exactly
    # truncgauss's construction. Only valid to compare where x_obs >= c
    # (otherwise X=Y>c is contradicted).
    mu0, c, sigma = 0.5, 2.0, 1.3
    for x_obs in [2.1, 2.5, 3.5, 3.9]:
        p = normal_carving_pvalue(x_obs, mu0, c, sigma, sigma, rho=1.0 - 1e-9)
        reference = truncgauss_pvalue(x_obs, mu0, c, scale=sigma)
...
def test_normal_carving_ci_rho_one_matches_truncgauss_ci():
    # rho->1 with sigma_x=sigma_y and x_obs >= c means X=Y, so this
    # matches truncgauss_ci's construction directly. x_obs is kept
    # comfortably above c (mild truncation) so the root search doesn't need
    # to probe deep into the joint tail, where scipy's generic bivariate
    # normal CDF loses all precision (both the numerator and denominator
    # genuinely underflow to 0 well before mu reaches the true root).
    sigma, alpha, x_obs, c = 1.3, 0.05, 4.0, 2.0
    lo, hi = normal_carving_ci(
        x_obs, c, alpha=alpha, sigma_x=sigma, sigma_y=sigma, rho=1 - 1e-9
    )
    lo_tg, hi_tg = truncgauss_ci(x_obs, c=c, alpha=alpha, scale=sigma)
    assert (lo, hi) == pytest.approx((lo_tg, hi_tg), abs=1e-6)
